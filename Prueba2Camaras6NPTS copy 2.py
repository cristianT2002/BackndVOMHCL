...
def procesar_frame_camara2(frame, results, hora_primera_deteccion_segundos_almacenado, 
    ...
    # print("la persona se detecta en:", detectado_persona.value)

    for result in results:
        for box, conf, cls in zip(result.boxes.xyxy, result.boxes.conf, result.boxes.cls):
            x_min, y_min, x_max, y_max = map(int, box)
            if cls == 1 and conf >= 0.1:
                detectado_persona.value = True

                cv2.rectangle(annotated_frame, (x_min, y_min), (x_max, y_max), (0, 255, 0), 2)
    with lock:
        if detectado_persona.value:
...
def logica_deteccion_personas(hora_primera_deteccion_segundos_almacenado, hora_sin_detecciones_segundos_almacenado, detectado_persona):
    global tiempos_alerta, timer_alerta, contador_alerta, hora_inicio_alerta, fecha_inicio_alerta, duracionAlertaTotalBD, tiempo_inicial_alerta
    ...
    while True:
        with lock:
            ...
            tormenta_npt(detectado_persona)
            alimentacion(hora_primera_deteccion_segundos_almacenado, hora_sin_detecciones_segundos_almacenado)
            gestionar_tiempos_npt(hora_primera_deteccion_segundos_almacenado, hora_sin_detecciones_segundos_almacenado, detectado_persona)
...
def tormenta_npt(detectado_persona):
    global tiempos_alerta, timer_alerta, contador_alerta, hora_inicio_alerta, fecha_inicio_alerta, duracionAlertaTotalBD, tiempo_inicial_alerta
    global mensaje_emitido_alerta, duracionAlertaTotal_en_minutos, duracionAlertaTotalBD, alerta  
    ...
    if timer_alerta:
        ...
        if duracionAlerta >= 30 and not mensaje_emitido_alerta and detectado_persona.value:
...
# Variables globales para controlar si ya se registró una parada corta o un NPT
registrado_parada_corta = False
registrado_otros_npt = False

# def gestionar_tiempos_npt(hora_primera_deteccion_segundos_almacenado, hora_sin_detecciones_segundos_almacenado, detectado_persona):
...
#     global tiempo_sin_deteccion
#     global registrado_parada_corta, registrado_otros_npt

#     # Cálculo del tiempo sin detección
#     tiempo_sin_deteccion = hora_primera_deteccion_segundos_almacenado.value - hora_sin_detecciones_segundos_almacenado.value
...
#                             registrado_parada_corta = True  # Marcar que ya se registró
#                             print("contador paradas cortas: ", contador_paradas_cortas)
#                             print(f"Parada corta registrada: {tiempo_sin_deteccion} segundos")

#                     else:  # Otros NPT
#                         # Solo registrar si no se ha registrado ya
#                         if not registrado_otros_npt:
#                             contador_otros_npt += 1
#                             tiempos_otros_npt.append(tiempo_sin_deteccion)
#                             registrado_otros_npt = True  # Marcar que ya se registró
#                             print("contador otros npt: ", contador_otros_npt)
#                             print(f"Otros NPT registrado: {tiempo_sin_deteccion} segundos")

#             # Resetear las banderas cuando la persona es detectada nuevamente
#             if detectado_persona.value == True:
#                 registrado_parada_corta = False
#                 registrado_otros_npt = False
...
registrado_parada_corta = False
registrado_otros_npt = False
tiempo_no_detectado_inicio = None  # Tiempo en el que la persona dejó de ser detectada
...
def gestionar_tiempos_npt(hora_primera_deteccion_segundos_almacenado, hora_sin_detecciones_segundos_almacenado, detectado_persona):
    ...
    global target_time_1_segundos_almuerzo, target_time_2_segundos_almuerzo
    global target_time_1_segundos, target_time_2_segundos
    global tiempo_sin_deteccion, registrado_parada_corta, registrado_otros_npt, tiempo_no_detectado_inicio
    ...
    if alerta not in ["2**", "3**"]:  # Excluir tormentas nivel 2** y 3**
        if isTiempos:  # Excluir tiempos de alimentación
            ...
            # Si la persona no es detectada
            if detectado_persona.value == False:
                # Si acaba de dejar de ser detectada, registrar el tiempo de inicio de no detección
                if tiempo_no_detectado_inicio is None:
                    tiempo_no_detectado_inicio = time.time()

                # Verificar si han pasado más de 2 segundos desde que se dejó de detectar
                if time.time() - tiempo_no_detectado_inicio > 2:
                    # Clasificación de tiempos fuera de horarios de alimentación
                    if not en_horario_desayuno and not en_horario_almuerzo:
                        # Clasificar el tiempo según duración
                        if tiempo_sin_deteccion > 0:
                            if 1 * 60 < tiempo_sin_deteccion <= 10 * 60:  # Parada corta si está entre 5 y 10 minutos
                                # Solo registrar si no se ha registrado ya
                                if not registrado_parada_corta:
                                    contador_paradas_cortas += 1
                                    tiempos_paradas_cortas.append(tiempo_sin_deteccion)
                                    ...
                                    print("contador paradas cortas: ", contador_paradas_cortas)
                                    print(f"Parada corta registrada: {tiempo_sin_deteccion } segundos")
                            elif tiempo_sin_deteccion > 10 * 60:  # Otros NPT si es mayor a 10 minutos
                                # Solo registrar si no se ha registrado ya
                                if not registrado_otros_npt:
                                    contador_otros_npt += 1
                                    tiempos_otros_npt.append(tiempo_sin_deteccion)
            ...
            # Si la persona es detectada nuevamente, resetear las banderas y el tiempo de no detección
            if detectado_persona.value == True:
                registrado_parada_corta = False
                registrado_otros_npt = False
                tiempo_no_detectado_inicio = None  # Reiniciar el tiempo de no detección
...
if __name__ == "__main__":
    ...
    detectado_persona = manager.Value('b', False)
